...
shinyServer(function(input, output) {
  ...
  output$plot4 <- renderPlot({
    ...
    times <- seq(0, w, by = 0.1)
    output1 <- dede(y = yinit2I, times = times, func = Ciupe2I, parms = parms4_2I)
    ...
    output2 <- dede(y = yinitR, times = times, func = CiupeR, parms = parms4_R)
    ...
    output3 <- dede(y = yinitA, times = times, func = CiupeA, parms = parms4_A)
    ...
    output4 <- dede(y = yinitAE, times = times, func = CiupeAE, parms = parms4_AE)
    ...
    if (input$AE == 1){
      ...
      vdisp[4] <- TRUE}
    pl <- pl + geom_point(patient4_obsV,mapping = aes(x = time, y = V), shape=8, color = "darkgreen", size=4)
    pl <- pl + labs(x='Time (Days)') + labs(y='HBV DNA (virions/mL)') +
      labs(color  = "Model", linetype = "Model")
